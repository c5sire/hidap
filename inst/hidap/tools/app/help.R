...
output$help_videos <- renderUI({
  file.path("..",app_dir,"tools/app/tutorials.md") %>% inclMD %>% HTML
})

output$help_about <- renderUI({
  file.path("..",app_dir,"tools/app/about.md") %>% inclMD %>% HTML
})
...
append_help <- function(help_str, help_path, Rmd = FALSE) {
  ...
  cc <- "&copy; International Potato Center (2015) <a rel='license' href='http://creativecommons.org/licenses/by-nc-sa/4.0/' target='_blank'><img alt='Creative Commons License' style='border-width:0' src ='imgs/80x15.png' /></a></br>"
  paste(all_help,"\n",mathjax_script,"\n",cc) %>% HTML
}

help_data <- c("Manage" = "manage.md","View" = "view.md", "Visualize" = "visualize.md",
               "Explore" = "explore.md", "Merge" = "merge.md", "Transform" = "transform.md")
output$help_data <- reactive(append_help("help_data", file.path("..",app_dir,"tools/help/")))
