...
shinyUI(navbarPage(app_title, id = "nav_radiant", inverse = TRUE,
  ...
  navbarMenu("Analysis",
             shiny::tabPanel(" Analysis of Variance (ANOVA)",uiOutput("single_anova"),icon = icon("font",lib="glyphicon")),
             shiny::tabPanel("Additive Main effects and Multiplicative Interaction (AMMI)",uiOutput("ammi"),icon = icon("cubes","fa-1x")),
